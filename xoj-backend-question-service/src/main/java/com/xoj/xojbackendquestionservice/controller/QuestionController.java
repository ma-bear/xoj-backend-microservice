package com.xoj.xojbackendquestionservice.controller;
...
import com.xoj.xojbackendcommon.common.DeleteRequest;
import com.xoj.xojbackendcommon.common.ErrorCode;
import com.xoj.xojbackendcommon.common.ResultUtils;
import com.xoj.xojbackendcommon.constant.UserConstant;
import com.xoj.xojbackendcommon.exception.BusinessException;
import com.xoj.xojbackendcommon.exception.ThrowUtils;
import com.xoj.xojbackendmodel.model.dto.Question.*;
...
public class QuestionController {
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        ...
        User loginUser = userOpenFeign.getLoginUser(request);
    ...
    public BaseResponse<Boolean> deleteQuestion(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        User user = userOpenFeign.getLoginUser(request);
        long id = deleteRequest.getId();
        // 判断是否存在
        Question oldQuestion = questionService.getById(id);
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可删除
        if (!oldQuestion.getUserId().equals(user.getId()) && !userOpenFeign.isAdmin(user)) {
    ...
    public BaseResponse<Question> getQuestionById(long id, HttpServletRequest request) {
        ...
        User loginUser = userOpenFeign.getLoginUser(request);
        // 不是本人或管理员，不能直接获取所有信息
        if (!question.getUserId().equals(loginUser.getId()) && !userOpenFeign.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
    ...
    public BaseResponse<Boolean> editQuestion(@RequestBody QuestionEditRequest questionEditRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldQuestion.getUserId().equals(loginUser.getId()) && !userOpenFeign.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        boolean result = questionService.updateById(question);
        return ResultUtils.success(result);
    ...
    public BaseResponse<Long> doQuestionSubmit(@RequestBody QuestionSubmitAddRequest questionSubmitAddRequest,
        ...
        // 登录才能提交
        final User loginUser = userOpenFeign.getLoginUser(request);
        long questionSubmitId = questionSubmitService.doQuestionSubmit(questionSubmitAddRequest, loginUser);
        return ResultUtils.success(questionSubmitId);
    }

    /**
     * 分页获取题目提交列表（除管理员外，用户只能看见公开信息）
     *
